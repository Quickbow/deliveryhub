...
export const SellerView = () => {
  ...
  // ALL state hooks must be declared before any conditional return (Rules of Hooks)
  ...
  // Synchronous auth guard — currentUser is read from localStorage on first render.
  ...
  const allOrders = orders.filter(order =>
    order.products.some(item => myProductIds.has(item.productId))
  );
